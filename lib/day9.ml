...
let parse_reading : string -> reading =
  function s ->
    String.split_on_char ' ' s
    |> List.map int_of_string
    |> Array.of_list

let parse_values : string list -> reading list =
  function lst ->
...
let n_choose_k : int -> int -> int =
  fun n k ->
    ...
    let c = if k > n - k 
      then 
        calc_range (k + 1) n 1 / Util.factorial (n - k)
      else
        calc_range (n - k + 1) n 1 / Util.factorial k
    in
...
let interpolate : reading -> int =
  function r ->
    let n = Array.length r in
    Array.mapi (fun i y_i -> compute_polynomial_term i y_i n) r
    |> (fun arr -> Printf.printf "interpolate: %s\n" (Util.string_of_int_array arr); arr)
    |> Util.sum_array
...
let interpolate_part_2 : reading -> int =
  function r ->
    let n = Array.length r in
...
let solve_part1 : string list -> string =
  function lst ->
    parse_values lst
    |> List.map interpolate
    |> (fun l -> print_endline (Util.string_of_int_list l); l)
    |> Util.sum_list
    |> string_of_int
