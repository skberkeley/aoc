...
let string_of_int_array : int array -> string =
  function arr ->
...
let array_index_of_opt : 'a -> 'a array -> int option =
  fun x arr ->
    ...
    |> Array.fold_left (fun so_far curr -> match so_far with | Some _ -> so_far | None -> curr) None
