...
let card_of_string : string -> card =
  fun s ->
    if Str.string_match card_re s 0 then
      int_list_of_string (Str.matched_group 1 s), int_list_of_string (Str.matched_group 2 s)
    else
      failwith "Card regex did not match"
...
let rec power_of_two : int -> int =
  fun n ->
    ...
    | 0 -> 0
    | 1 -> 1
    | n -> 2 * power_of_two (n - 1)

let num_winning_numbers : card -> int =
  function winning, my_nums ->
    let winning_num_set = Int_set.of_list winning in
    num_in_set winning_num_set my_nums

let score_card : card -> int =
  function c ->
    num_winning_numbers c
    |>  power_of_two
...
let rec calculate_num_cards : int Int_map.t -> int -> int -> card list -> int =
  fun num_card_copies total i cards ->
    ...
    | c :: lst ->
      ...
      let updated_num_copies = List.map (fun n -> (n, (Int_map.find n num_card_copies) + Int_map.find i num_card_copies)) copies_won |> List.to_seq in
      let num_card_copies = Int_map.add_seq updated_num_copies num_card_copies in
      calculate_num_cards num_card_copies (total + (Int_map.find i num_card_copies)) (i + 1) lst

let solve_part2 : string list -> string =
  fun lst ->
    let starting_counts = range 0 (List.length lst - 1) |> List.map (fun i -> (i, 1)) |> List.to_seq |> Int_map.of_seq in
    List.map card_of_string lst
