...
let int_of_hand_type : hand_type -> int = function
  | Five_oak ->
      7
  | Four_oak ->
      6
...
let compare_hand_types : hand_type -> hand_type -> int =
 fun h1 h2 -> compare (int_of_hand_type h1) (int_of_hand_type h2)
...
module Card = struct
  ...
  let card_of_char : char -> t = function
    | 'A' ->
        A
    | 'K' ->
        K
    ...
    | 'T' ->
        T
    | '1' .. '9' as n ->
        let n = String.make 1 n |> int_of_string in
        Num n
    | _ ->
        failwith "Unrecognized character"

  let int_of_card : t -> int = function
    | A ->
        14
    | K ->
        13
  ...
  let compare : t -> t -> int =
   fun c1 c2 -> compare (int_of_card c1) (int_of_card c2)

  let rec compare_seqs : t Seq.t -> t Seq.t -> int =
   fun s1 s2 ->
    match (s1 (), s2 ()) with
    | Cons (c1, r1), Cons (c2, r2) -> (
      match compare c1 c2 with 0 -> compare_seqs r1 r2 | c -> c )
    | Nil, Nil ->
        0
    | _ ->
  ...
  let compare_arrays : t array -> t array -> int =
   fun arr1 arr2 -> compare_seqs (Array.to_seq arr1) (Array.to_seq arr2)

  let string_of_card : t -> string = function
    | A ->
        "A"
    | K ->
        "K"
...
let string_of_int_cardmap : int CardMap.t -> string = function
  | card_map ->
      CardMap.to_seq card_map |> List.of_seq
      |> List.map (function c, n ->
             Printf.sprintf "%s: %d" (Card.string_of_card c) n )
      |> String.concat ", "
...
let compare_hands : hand -> hand -> int =
 fun (t1, arr1, _) (t2, arr2, _) ->
  ...
  | 0 ->
      Card.compare_arrays arr1 arr2
  | c ->
      c

let string_of_hand : hand -> string = function
  | hand_type, cards, bid ->
      let cards =
        Array.map Card.string_of_card cards |> Array.to_list |> String.concat ""
      in
      Printf.sprintf "%s %s %d" (string_of_hand_type hand_type) cards bid

type unclassified_hand = Card.t array * int

let string_of_unclassified_hand : unclassified_hand -> string = function
  | cards, bid ->
      let cards =
        Array.map Card.string_of_card cards |> Array.to_list |> String.concat ""
      in
...
let parse_cards : string -> Card.t array =
 fun s -> String.to_seq s |> Seq.map Card.card_of_char |> Array.of_seq

let parse_hand : string -> unclassified_hand =
 fun s ->
  if not (Str.string_match hand_regex s 0) then
    Printf.sprintf "Couldn't parse hand %s" s |> failwith
  else
...
let rec card_map_of_cards : Card.t list -> int CardMap.t = function
  | [] ->
      CardMap.empty
  | c :: l ->
      let m = card_map_of_cards l in
      if CardMap.mem c m then CardMap.add c (CardMap.find c m + 1) m
      else CardMap.add c 1 m

let classify_hand_from_counts : int CardMap.t -> hand_type = function
  | card_map -> (
      ...
      | 1 ->
          Five_oak
      | 2 ->
          if CardMap.exists (fun _ i -> i = 4) card_map then Four_oak
          else Full_house
      | 3 ->
          if CardMap.exists (fun _ i -> i = 3) card_map then Three_oak
          else Two_pair
      | 4 ->
          One_pair
      | 5 ->
          High_card
      | _ ->
          failwith "Unexpected number of card counts" )
...
let most_common_card : int CardMap.t -> Card.t = function
  | card_map ->
      CardMap.fold
        (fun card count so_far ->
          ...
          else if count > so_far_count then card
          else if
            count = so_far_count
            && Card.int_of_card card > Card.int_of_card so_far
          then card
          else so_far )
        card_map
        ((function c, _ -> c) (CardMap.choose card_map))

let remove_jokers_from_card_map : int CardMap.t -> int CardMap.t = function
  | card_map ->
      if CardMap.cardinal card_map = 1 && CardMap.mem J card_map then card_map
      else
        let common_card = most_common_card card_map in
        CardMap.add common_card
          (CardMap.find common_card card_map + CardMap.find J card_map)
          card_map
        |> CardMap.remove J
...
let classify_hand_with_jokers : unclassified_hand -> hand =
  ...
  | cards, bid ->
      ...
      let card_map =
        if CardMap.mem J card_map then remove_jokers_from_card_map card_map
        else card_map
      in
      (classify_hand_from_counts card_map, cards, bid)
...
let solve_part1 : string list -> string =
 fun lst ->
  ...
  parse_hands lst |> List.map classify_hand |> List.sort compare_hands
