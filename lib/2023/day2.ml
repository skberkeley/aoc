...
let handful_valid : handful -> bool =
  fun h ->
    h.red <= bag.red && h.blue <= bag.blue && h.green <= bag.green

let game_valid : game -> bool =
  fun g ->
    List.for_all handful_valid g.handfuls
...
let handful_of_string : string -> handful =
  fun s ->
    let rec helper : handful -> string -> handful =
      fun h s ->
        if String.length s = 0 then h else
        match parse_color_count s with
        | color, n, s' ->
          if color = "red" then
            helper {red = n; blue = h.blue; green = h.green} s'
          else if color = "blue" then
            helper {red = h.red; blue = n; green = h.green} s'
          else
            helper {red = h.red; blue = h.blue; green = n} s'
    in
...
let rec min_set_of_handfuls : handful list -> handful -> handful =
  fun lst h ->
   ...
   | {red = r; blue = b; green = g} :: lst ->
    min_set_of_handfuls lst {red = max h.red r; blue = max h.blue b; green = max h.green g}

let min_set : game -> handful =
  fun g -> 
    min_set_of_handfuls g.handfuls {red = 0; blue = 0; green = 0}

let power_of_handful : handful -> int =
  fun h ->
    h.red * h.blue * h.green

let solve_part2 : string list -> string =
  fun lst ->
    List.map game_of_string lst 
    |> List.map min_set 
    |> List.map power_of_handful
    |> List.fold_left (+) 0
    |> string_of_int
