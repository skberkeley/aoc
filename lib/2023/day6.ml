...
let parse_races : string list -> race list =
  fun lst ->
    if List.length lst <> 2 then failwith "input was of unexpected length" else
    let time_string = List.hd lst in
    let distance_string = List.hd (List.tl lst) in
    if not (Str.string_match times_regex time_string 0) then Printf.sprintf "time_regex didn't match %s\n" time_string |> failwith else
    if not (Str.string_match distance_regex distance_string 0) then Printf.sprintf "distance_regex didn't match %s\n" distance_string |> failwith else
    let times = Str.matched_group 1 time_string |> int_list_of_string in
    let distances = Str.matched_group 1 distance_string |> int_list_of_string in
...
let solve_part1 : string list -> string =
  fun lst ->
    parse_races lst
    |> List.map num_winning_ways
    |> List.fold_left ( * ) 1
    |> string_of_int

let int_of_spaced_string : string -> int =
  function s -> 
    String.split_on_char ' ' s 
    |> List.filter (fun s -> String.length s > 0) 
    |> String.concat ""
...
let parse_race : string list -> race =
  function
  | time_string :: distance_string :: [] ->
    if not (Str.string_match times_regex time_string 0) then Printf.sprintf "time_regex didn't match %s\n" time_string |> failwith else
    if not (Str.string_match distance_regex distance_string 0) then Printf.sprintf "distance_regex didn't match %s\n" distance_string |> failwith else
    let time = Str.matched_group 1 time_string |> int_of_spaced_string in
    let distance = Str.matched_group 1 distance_string |> int_of_spaced_string in
    time, distance
  | _ -> failwith "input was of unexpected length"
