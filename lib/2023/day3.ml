...
let adjacent_symbol_present : bytes array -> int -> int -> int -> bool =
  fun arr i j len ->
    ...
    if j + len < Bytes.length row then begin
    ...
    for i = 0 to Array.length arr - 1 do
      ...
      for j = 0 to Bytes.length row - 1 do
        ...
        if !digits_remaining <> 0 then
          ...
          if adjacent_symbol_present arr i j length then
            nums := num :: !nums;
...
let find_adjacent_nums : bytes array -> int -> int -> int list =
  fun arr i j ->
    ...
    if i > 0 then begin
      ...
      if j - 1 > 0 then begin
        if is_digit (Bytes.get row (j - 1)) then begin
          nums := get_num arr (i - 1) (j - 1) :: !nums;
          !has_digit.(0) <- true;
        end
      end;
      if is_digit (Bytes.get row j) then begin
        !has_digit.(1) <- true;
        if not !has_digit.(0) then begin
          nums := get_num arr (i - 1) j :: !nums
        end
      end;
      if is_digit (Bytes.get row (j + 1)) then begin
        if not !has_digit.(1) then begin
          nums := get_num arr (i - 1) (j + 1) :: !nums
        end;
      end;
    end;
